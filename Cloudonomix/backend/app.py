...
def insights():
    vms = get_vm_data()
    result = analyze_resources(vms)
    return jsonify(result)

from flask import request

from backend.data import VM_DATA

@app.route("/api/shutdown-vm", methods=["POST"])
def shutdown_vm():
    vm_id = request.json.get("vm_id")

    for vm in VM_DATA:
        if vm["vm_id"] == vm_id and vm["status"] == "running":
            vm["status"] = "stopped"
            vm["cpu_usage"] = 0
            vm["hours_running"] = 0

            return {
                "message": f"VM {vm_id} shutdown successfully",
                "status": "stopped"
            }

    return {"error": "VM not found or already stopped"}, 404
